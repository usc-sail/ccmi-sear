...
class ASTModel(nn.Module):
    def __init__(self, label_dim=527,
                 ...
                 input_prompt=False, embedding_prompt=False, adapter=False,
                 num_inp_prompts=10, num_emb_prompts=4):

        super(ASTModel, self).__init__()
        assert timm.__version__ == '0.4.5', 'Please use timm == 0.4.5, the code might not be compatible with newer versions.'
        assert adapter == False, "Adapter implementation disabled"

        # override timm input shape restriction
        timm.models.vision_transformer.PatchEmbed = PatchEmbed

        # pretrain the AST models
#        no_ft = (input_prompt or embedding_prompt or adapter)
        if pretrain_stage == True or (pretrain_stage==False and load_pretrained_mdl_path==None):
...
#            vkwargs = {'adapter':adapter}
            if model_size == 'tiny':
                self.v = timm.create_model('vit_deit_tiny_distilled_patch17_224', pretrained=False)
            ...
            self.temb_dim, self.femb_dim = 256, 256
            # embedding layer for frequency patch alignment
#            self.fpredlayer = nn.Sequential(nn.Linear(self.original_embedding_dim, self.original_embedding_dim), nn.ReLU(), nn.Linear(self.original_embedding_dim, self.femb_dim))
            # embedding layer for temporal patch alignment
 #           self.tpredlayer = nn.Sequential(nn.Linear(self.original_embedding_dim, self.original_embedding_dim), nn.ReLU(), nn.Linear(self.original_embedding_dim, self.temb_dim))
            ...
            self.num_prompt_tokens = num_emb_prompts
           ...
           #     raise  ValueError('The model loaded is not from a torch.nn.Dataparallel object. Wrap it with torch.nn.Dataparallel and try again.')
            ...
            sd = {k:v for k,v in sd.items() if 'mlp_head' not in k}
            ...
            print("missing keys, training these:", msg.missing_keys)
            if input_prompt or embedding_prompt or adapter:
            ...
            if self.input_prompt:
                self.input_prompter = Input_prompt(prompt_size=num_inp_prompts, input_tdim = input_tdim, input_fdim = input_fdim)
            if self.embedding_prompt:
#                self.embedding_prompt_tokens = {}
                for i in range(12):
                    setattr(self, 'embedding_prompt_tokens{}'.format(i), nn.Parameter(torch.empty(1, num_emb_prompts, self.original_embedding_dim)))
                    torch.nn.init.xavier_uniform_(getattr(self, 'embedding_prompt_tokens{}'.format(i)).data)
...
#        num_blocks = perm_size // (blockwidth ** 2)
    ...
    def finetuningcls(self, x):
        ...
        if self.input_prompt:
            ...
            x = self.input_prompter(x).unsqueeze(1).transpose(-1, -2)
        ...
        for blk_id, blk in enumerate(self.v.blocks):
            if self.embedding_prompt:
                tuning_prompts = getattr(self, 'embedding_prompt_tokens{}'.format(blk_id)).expand(B, -1, -1)
                x = torch.cat([tuning_prompts, x], dim=1)
    ...
    def jigsaw(self, x, mask_patch, blocksize=3, reconstruct=False, show_perm=False):
        input = self.unfold(x).transpose(1,2)
        B = x.shape[0]
        P = input.shape[1]
        x_orig = input.clone().detach()
        
        x = self.v.patch_embed(x)

        x_shuffle = torch.empty(x.shape, device=x.device)
        perm_mask = torch.empty(x.shape[:-1], device=x.device).bool()
        perm_seq = torch.empty(x.shape[:-1]).long()
        num_shuf = torch.empty(B, device=x.device).int()
        perm_size = torch.empty(B, device=x.device).int()

        for i in range(B):
            perm_seq[i], perm_size[i], num_shuf[i] = self.gen_permid_patch(P, perm_size=mask_patch, blocksize=blocksize)
            x_shuffle[i] = x[i][perm_seq[i]]
            perm_mask[i] = torch.ne(perm_seq[i], torch.arange(P))

        
        # pass through the Transformer layers
        cls_tokens = self.v.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        dist_token = self.v.dist_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens, dist_token, x_shuffle), dim=1)
        x = x + self.v.pos_embed
        x = self.v.pos_drop(x)
        ...
        if not show_perm:
            ...
            for i in np.arange(0, B):
                perm_ids = torch.arange(P)[perm_mask[i]] 
                pred = self.cpredlayer(x[i,perm_ids+2,:]) # cls tokens
                orig = x_orig[i,perm_ids,:]
                total = torch.mm(orig, torch.transpose(pred, 0, 1))  # e.g. size 100*100
                correct += (torch.sum(torch.eq(torch.argmax(self.softmax(total), dim=0), torch.arange(0, len(perm_ids), device=x.device))) / len(perm_ids))  # correct is a tensor
                nce += (torch.sum(torch.diag(self.lsoftmax(total))) / len(perm_ids))  # nce is a tensor
            acc = 1. * correct / (B)
            nce = nce / (-1. * B)
                
            if reconstruct:
                pred_patch = torch.empty(x_orig.shape).to(x.device) 
                for i in range(B):
                    pred_patch[i] = self.gpredlayer(x[i,2:,:])
                mse = torch.mean((pred_patch - input) ** 2).to(x.device)
                return acc, nce, mse, perm_size, num_shuf
        
            return acc, nce, perm_size, num_shuf
        else:
            x_cont = x_orig.clone()
            x_shuf = x_orig.clone()
            ...
            for i in range(B):
                perm_ids = torch.arange(P)[perm_mask[i]]
                x_cont[i,perm_ids,:] = self.cpredlayer(x[i, perm_ids+2,:])
                x_shuf[i] = x_orig[i][perm_seq[i]]
            
            if reconstruct:
                recon = torch.empty(x_orig.shape).to(x.device)
                for i in range(B):
                    recon[i] = self.gpredlayer(x[i,2:,:])
                return x_orig, x_shuf, x_cont, recon

            return x_orig, x_shuf, x_cont
    ...
    def forward(self, x, task, cluster=True, blocksize=3, mask_patch=400):
        ...
        elif task == 'pretrain_jigsaw':
            return self.jigsaw(x, mask_patch=mask_patch)
        elif task == 'pretrain_jigsaw_joint':
            return self.jigsaw(x, mask_patch=mask_patch, blocksize=blocksize, reconstruct=True)
        elif task == 'pretrain_joint':
            return self.joint_masked(x, mask_patch=mask_patch, cluster=cluster)
        else:
